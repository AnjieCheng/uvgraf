...
from src.training.inference_utils import generate_cano, generate_trajectory, generate, generate_camera_angles, generate_trajectory_cano
...
def generate_vis(cfg: DictConfig):
    ...
    geo_ws, tex_ws = sample_ws_from_seeds(G, [17], cfg, device, num_interp_steps=0)
    tex_ws = tex_ws.reshape(1, tex_ws.shape[1], *tex_ws.shape[2:])

    if cfg.vis.name == 'front_grid':
        geo_ws, _ = sample_ws_from_seeds(G, seeds, cfg, device, num_interp_steps=0) # [num_videos, num_ws, w_dim]
        geo_ws = geo_ws.reshape(len(seeds), geo_ws.shape[1], *geo_ws.shape[2:]) # [num_frames * num_videos, num_ws, w_dim]
        # tex_ws = tex_ws.reshape(len(seeds), tex_ws.shape[1], *tex_ws.shape[2:]) # [num_frames * num_videos, num_ws, w_dim]
        tex_ws = tex_ws.expand(len(seeds), -1, -1)

        trajectory = generate_camera_angles(cfg.camera, default_fov=None)[0][:, :2] # [num_grids, 2]
        images, _ = generate_trajectory_cano(cfg, G, geo_ws, tex_ws, trajectory) # [num_frames, num_videos, c, h, w]
        images = images.permute(1, 0, 2, 3, 4)[:,:,:3,:,:] # [num_samples, num_traj_steps, c, h, w]
    ...
    elif cfg.vis.name == 'tex_transfer':
        trajectory = torch.tensor([0.0, np.pi * 3 / 8]).unsqueeze(0).numpy() # [1, 2]
        geo_ws, tex_ws = sample_ws_from_seeds(G, seeds, cfg, device, num_interp_steps=0) # [num_videos, num_ws, w_dim]
        geo_ws = geo_ws.reshape(geo_ws.shape[1], *geo_ws.shape[2:]) # [num_frames * num_videos, num_ws, w_dim]
        tex_ws = tex_ws.reshape(tex_ws.shape[1], *tex_ws.shape[2:]) # [num_frames * num_videos, num_ws, w_dim]

        images = generate_trajectory_cano(cfg, G, geo_ws, tex_ws, trajectory, fovs=G.cfg.dataset.sampling.fov, **cfg.synthesis_kwargs)
        ...
        for seed_from, seed_to, grid in tqdm(list(zip(seeds[0::2], seeds[1::2], images)), desc='Saving'):
            ...
            tv.io.write_video(save_path, video, fps=cfg.vis.fps, video_codec='h264', options={'crf': '10'})
    elif cfg.vis.name == 'interp_video_tex':
        """Does 'live' interpolation, i.e. an object rotates and transforms into another one simuletaneously"""
        # import pdb; pdb.set_trace()
        num_videos = len(seeds) // 2 # [1]
        angles = generate_camera_angles(cfg.camera, default_fov=None)[0] # [num_videos, 3]
        geo_ws, tex_ws = sample_ws_from_seeds(G, seeds, cfg, device, num_interp_steps=cfg.camera.num_frames, target='tex') # [num_frames, num_videos, num_ws, w_dim]
        geo_ws = geo_ws.reshape(cfg.camera.num_frames * num_videos, *geo_ws.shape[2:]) # [num_frames * num_videos, num_ws, w_dim]
        tex_ws = tex_ws.reshape(cfg.camera.num_frames * num_videos, *tex_ws.shape[2:]) # [num_frames * num_videos, num_ws, w_dim]
        
        angles = torch.from_numpy(angles).float().to(device).repeat_interleave(num_videos, dim=0) # [num_frames * num_videos, 3]
        images = generate_cano(cfg, G, geo_ws, tex_ws, angles=angles) # [num_frames * num_videos, c, h, w]
        images = images.reshape(cfg.camera.num_frames, num_videos, *images.shape[1:]) # [num_frames, num_videos, c, h, w]
        images = images.permute(1, 0, 2, 3, 4) # [num_videos, num_frames, c, h, w]
        for seed_from, seed_to, video in tqdm(list(zip(seeds[0::2], seeds[1::2], images)), desc='Saving'):
            save_path = os.path.join(save_dir, f'seeds-{seed_from:04d}-{seed_to:04d}.mp4')
    ...
    elif cfg.vis.name == 'interp_video_geo':
        """Does 'live' interpolation, i.e. an object rotates and transforms into another one simuletaneously"""
        # import pdb; pdb.set_trace()
        num_videos = len(seeds) // 2 # [1]
        angles = generate_camera_angles(cfg.camera, default_fov=None)[0] # [num_videos, 3]
        geo_ws, tex_ws = sample_ws_from_seeds(G, seeds, cfg, device, num_interp_steps=cfg.camera.num_frames, target='geo') # [num_frames, num_videos, num_ws, w_dim]
        geo_ws = geo_ws.reshape(cfg.camera.num_frames * num_videos, *geo_ws.shape[2:]) # [num_frames * num_videos, num_ws, w_dim]
        tex_ws = tex_ws.reshape(cfg.camera.num_frames * num_videos, *tex_ws.shape[2:]) # [num_frames * num_videos, num_ws, w_dim]
        ...
        images = generate_trajectory_cano(cfg, G, geo_ws, tex_ws, angles[:, :2], fovs=fovs, **cfg.synthesis_kwargs) # [num_frames, num_videos, c, h, w]
...
def sample_ws_from_seeds(G, seeds: List[int], cfg: DictConfig, device: str, num_interp_steps: int=0, target: str=None):
    ...
    if num_interp_steps == 0:
        z0 = sample_z_from_seeds([8888]*len(seeds), G.z_dim).to(device) # [num_samples, z_dim]
        ...
        geo_z = z[...,:G.geo_dim]
        tex_z = z0[...,-G.tex_dim:]
        geo_ws = G.geo_mapping(geo_z, c, truncation_psi=cfg.truncation_psi) # [num_videos, num_ws, w_dim]
        tex_ws = G.tex_mapping(tex_z, c, truncation_psi=cfg.truncation_psi) # [num_videos, num_ws, w_dim]
        return (geo_ws, tex_ws)
    else:
        ...
        c = sample_c_from_seeds(seeds[0::2], G.c_dim).to(device) # [num_samples, c_dim]
        # c_to = sample_c_from_seeds(seeds[1::2], G.c_dim).to(device) # [num_samples, c_dim]
        geo_z_from = z_from[...,:G.geo_dim]
        tex_z_from = z_from[...,-G.tex_dim:]
        geo_z_to = z_to[...,:G.geo_dim]
        tex_z_to = z_to[...,-G.tex_dim:]

        geo_ws_from = G.geo_mapping(geo_z_from, c, truncation_psi=cfg.truncation_psi) # [num_videos, num_ws, w_dim]
        tex_ws_from = G.tex_mapping(tex_z_from, c, truncation_psi=cfg.truncation_psi) # [num_videos, num_ws, w_dim]

        geo_ws_to = G.geo_mapping(geo_z_to, c, truncation_psi=cfg.truncation_psi) # [num_videos, num_ws, w_dim]
        tex_ws_to = G.tex_mapping(tex_z_to, c, truncation_psi=cfg.truncation_psi) # [num_videos, num_ws, w_dim]
        alpha = torch.linspace(0, 1, num_interp_steps, device=device).view(num_interp_steps, 1, 1, 1) # [num_interp_steps]

        if target == 'tex':
            geo_ws = geo_ws_from.unsqueeze(0) * (1 - alpha*0) + geo_ws_to.unsqueeze(0) * alpha * 0 # [num_interp_steps, num_samples, num_ws, w_dim]
            tex_ws = tex_ws_from.unsqueeze(0) * (1 - alpha) + tex_ws_to.unsqueeze(0) * alpha # [num_interp_steps, num_samples, num_ws, w_dim]
        else: # geo
            geo_ws = geo_ws_from.unsqueeze(0) * (1 - alpha) + geo_ws_to.unsqueeze(0) * alpha # [num_interp_steps, num_samples, num_ws, w_dim]
            # tex_ws = tex_ws_from.unsqueeze(0) * (1 - alpha) + tex_ws_to.unsqueeze(0) * alpha # [num_interp_steps, num_samples, num_ws, w_dim]
            tex_ws = tex_ws_from.unsqueeze(0) * (1 - alpha*0) + tex_ws_to.unsqueeze(0) * alpha * 0 # [num_interp_steps, num_samples, num_ws, w_dim]
        return (geo_ws, tex_ws)

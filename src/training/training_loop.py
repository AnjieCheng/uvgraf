...
def training_loop(
    ...
    if not resume_pkl is None and (rank == 0):
    ...
    # resume_geo_pkl = "/data/anjie/Projects/epigraf/experiments/carla_64_canograf_patch_beta_p16_mins0.25_sanity_ours_tanh2_2gpu_reimp-0fb6cdd/output/network-snapshot-002016.pkl"
    ...
    if rank == 0:
        if not resume_whole_state:
            ...
            save_filename_mask = 'fakes_init_mask.jpg'
        else:
        ...
        # save_videos(tex_videos, os.path.join(run_dir, save_filename.replace('.jpg', '_tex.mp4')))
    else:
        vis = dnnlib.EasyDict()
    ...
    while True:
        ...
        if (rank == 0) and (cfg.training.image_snap is not None) and (done or cur_tick % cfg.training.image_snap == 0):
            ...
            # save_videos(tex_videos, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_tex.mp4'))

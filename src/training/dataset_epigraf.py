    ...
    def __init__(self,
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        if len(dataset_desc_files) == 0:
            print('no dataset json found, skip filtering')
        else:
            ...
            with self._open_file(dataset_desc_files[0]) as f:
                values = json.load(f).get('camera_angles')
                valid_file_list = []
                for i_name in self._image_fnames:
                    if i_name in values.keys():
                        valid_file_list.append(i_name)
                    else:
                        print("file: ", i_name, " not found in json... skipping...")
                self._image_fnames = valid_file_list

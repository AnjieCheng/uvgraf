...
# def canonical_renderer_pretrain(tex_x: torch.Tensor, geo_x: torch.Tensor, uv_x: torch.Tensor, coords: torch.Tensor, ray_d_world: torch.Tensor, mlp_f: Callable, mlp_b: Callable, template: Callable, texture_mlp: Callable, geo_mlp: Callable, scale: float=1.0, rt_sdf: bool=False) -> torch.Tensor:
#     # with torch.no_grad():
...
#     geo_x_raw = geo_x.view(batch_size * 3, feat_dim, h, w) # [batch_size * 3, feat_dim, h, w]
#     coords = coords / scale # [batch_size, num_points, 3]
#     coords_2d = torch.stack([
#         coords[..., [0, 1]], # x/y plane
#         coords[..., [0, 2]], # x/z plane
...
#     coords_2d = coords_2d.view(batch_size * 3, 1, num_points, 2) # [batch_size * 3, 1, num_points, 2]
#     # assert ((coords_2d.min().item() >= -1.0 - 1e-8) and (coords_2d.max().item() <= 1.0 + 1e-8))
#     geo_x = F.grid_sample(geo_x_raw, grid=coords_2d, mode='bilinear', align_corners=True).view(batch_size, 3, feat_dim, num_points) # [batch_size, 3, feat_dim, num_points]
...
#     coords_f = F.tanh(mlp_output[..., -3:])  # F.tanh()
#     # rgbs = mlp_output[..., 0:3]
#     sigmas, sdfs = template(coords_f, get_sdf=True)
#     # coords_b = mlp_b(geo_x, coords_f, ray_d_world)  # F.tanh() * scale
...
#     # backward with coord_f
#     geo_x_f = F.grid_sample(geo_x_raw, grid=coords_f_2d, mode='bilinear', align_corners=True).view(batch_size, 3, feat_dim, num_points) # [batch_size, 3, feat_dim, num_points]
#     geo_x_f = geo_x_f.permute(0, 1, 3, 2) # [batch_size, 3, num_points, feat_dim]
#     coords_b = F.tanh(mlp_b(geo_x_f, coords_f, ray_d_world)) * scale # F.tanh() * scale

#     # og rgbs (start)
#     uv_x = uv_x.view(batch_size * 3, feat_dim, h, w) # [batch_size, feat_dim, h, w]
#     uv_x = F.grid_sample(uv_x, grid=coords_f_2d, mode='bilinear', align_corners=True).view(batch_size, 3, feat_dim, num_points) # [batch_size, 3, feat_dim, num_points]
#     uv_x = uv_x.permute(0, 1, 3, 2) # [batch_size, 3, num_points, feat_dim]
#     rgbs = texture_mlp(uv_x, coords_f, ray_d_world) # [batch_size, num_points, out_dim]
#     # og rgbs (ends)
...
def forward_uniform(geo_ws: torch.Tensor, tex_x: torch.Tensor, geo_x: torch.Tensor, coords: torch.Tensor, ray_d_world: torch.Tensor, mlp_f: Callable, mlp_b: Callable, template: Callable, texture_mlp: Callable, scale: float=1.0, rt_sdf: bool=False) -> torch.Tensor:
    # geo
    # with torch.no_grad():
    assert geo_x.shape[1] % 3 == 0, f"We use 3 planes: {geo_x.shape}"
    batch_size, raw_feat_dim, h, w = geo_x.shape

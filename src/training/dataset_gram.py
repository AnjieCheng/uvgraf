...
from src.training.dataset_utils import read_pose, read_pose_npy, transform_matrix_to_camera_pos
...
class Dataset(torch.utils.data.Dataset):
    ...
    def get_camera_angles(self, idx) -> Tuple[float]:
        if self._name == 'ffhq_seg':
            return read_pose(self._pose_fnames[self._raw_idx[idx]], flip=self._xflip[idx])
        elif self._name == 'carla':
            return transform_matrix_to_camera_pos(np.load(self._pose_fnames[self._raw_idx[idx]]), flip=self._xflip[idx])
        elif self._name == 'cat':
            return read_pose_npy(self._pose_fnames[self._raw_idx[idx]], flip=self._xflip[idx])
        else:
            raise NotImplementedError
        """
...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        if name == 'ffhq_seg':
            self._pose_fnames = [os.path.join('data/ffhq_seg/poses',f.split('/')[-1].replace('_mask.png','.mat')) for f in self._image_fnames]
        elif name == 'cats':
            self._pose_fnames = [os.path.join('data/cats/poses',f.split('/')[-1].replace('.png','_pose.npy')) for f in self._image_fnames]
        elif name == 'carla':
            self._pose_fnames = [os.path.join('data/carla/poses',f.split('/')[-1].replace('.png','_extrinsics.npy')) for f in self._image_fnames]
        else:
            raise NotImplementedError
        ...
        if resolution is not None and (raw_shape[2] != resolution or raw_shape[3] != resolution):
            ...
            print('Image files do not match the specified resolution. Will Resize!')
    ...
    def _load_raw_image(self, raw_idx):
        ...
        if self._resolution is not None and (image.shape[0] != self._resolution or image.shape[1] != self._resolution):
            image = cv2.resize(image, (self._resolution, self._resolution), interpolation=cv2.INTER_LINEAR)

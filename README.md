...
```python
CUDA_VISIBLE_DEVICES=5 python scripts/extract_geometry.py hydra.run.dir=. ckpt.network_pkl="/data/anjie/Projects/epigraf/experiments/shapenet_64_epigraf_patch_beta_p32_mins0.5_shapenet_dev_64_tri128-df22e28/output/network-snapshot-001242.pkl"
CUDA_VISIBLE_DEVICES=5 python scripts/inference.py hydra.run.dir=. ckpt.network_pkl="/data/anjie/Projects/epigraf/experiments/shapenet_64_epigraf_patch_beta_p32_mins0.5_shapenet_dev_64_tri128-df22e28/output/network-snapshot-001258.pkl" vis=video camera=front_circle output_dir="./videos" num_seeds=16 truncation_psi=0.7
...
```bash
docker run --gpus device=all -it --rm -v YOUR_LOCAL_FOLDER:MOUNT_FOLDER -it dis3d:v1 bash
```

* modify the resolution of all images in a folder
```bash
...
* extract geometry
```
CUDA_VISIBLE_DEVICES=5 python scripts/extract_geometry.py hydra.run.dir=. ckpt.network_pkl=/data/anjie/Projects/epigraf/experiments/carla_64_epigraf_patch_beta_p16_mins0.25_carla_fast_64_ft64-df22e28/output/network-snapshot-009007.pkl
```

* train holder for two gpus
```
CUDA_VISIBLE_DEVICES=4,7 python src/infra/launch.py hydra.run.dir=. exp_suffix=holder dataset=shapenet dataset.resolution=64 num_gpus=2 training.batch_size=120
```

* train my model
```
CUDA_VISIBLE_DEVICES=5,6 python src/infra/launch.py hydra.run.dir=. exp_suffix=canograf_dev dataset=carla dataset.resolution=64 num_gpus=2 training.batch_size=90 model=canograf
```
